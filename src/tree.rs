...
impl Env {
    ...
    pub fn define(&mut self, name: impl AsRef<str>, val: impl Into<Value>) {
        self.inner.as_ref().borrow_mut().define(name.as_ref(), val);
        self.cursor += 1;
    }
...
mod tests {
    ...
    fn test_cursor() {
        ...
        let env2: Env = env.with_cursor(0);
        assert!(env2.get(&id("foo")).unwrap_err().is_not_found());
    }
